...
def test_get_badges_already_received(mock_session, mock_badge_already_received):
    # Arrange
    user_id = 1
    mock_session.exec.return_value.all.return_value = [(mock_badge_already_received, datetime.now())]

    # Act
    result = get_badges_already_received(user_id, mock_session)
...
def test_get_criteria_for_unawarded_badges(mock_session, mock_badge_criteria, mock_badge_already_received, mock_badge_not_received):
    # Arrange
    awarded_badge_ids = {mock_badge_already_received.id}
    mock_session.exec.return_value.all.return_value = [mock_badge_criteria]

    # Act
    result = get_criteria_for_unawarded_badges(awarded_badge_ids, mock_session)
...
def test_award_badge(mock_session, mock_user_badge_to_award):
    # Arrange
    user_id = 1
    badge_id = 1
    ...
    # Act
    result = award_badge(user_id, badge_id, mock_session)
...
def test_evaluate_criteria_and_type(mock_session, mock_badge_and_criteria):
    # Arrange
    user_id = 1
...
def test_evaluate_criteria_or_type(mock_session, mock_badge_or_criteria):
    # Arrange
    user_id = 1
...
def test_evaluate_criteria_unknown_type(mock_session):
    # Arrange
    user_id = 1
    ...
    with pytest.raises(ValueError, match="Unknown criteria type: unknown_type"):
        evaluate_criteria(user_id, criteria, mock_session)


def test_get_user_badges_returns_already_awarded_badges_and_newly_received_ones(
        mock_session,
    ...
    get_badge_mock = mock_session.get.return_value = mock_badge_not_received

    # Act
    result = get_user_badges(user_id, mock_session)
