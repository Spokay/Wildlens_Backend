...
async def test_check_image_for_footprint_returns_false_when_threshold_is_not_met(
    ...
    result = await wildlens_prediction_api_service.check_image_for_footprint(valid_image_file)

    assert result is False

@pytest.mark.asyncio
async def test_check_image_for_footprint_returns_true_when_threshold_is_met(
        wildlens_prediction_api_service,
        valid_image_file,
        mocker
):
...
@pytest.mark.asyncio
async def test_classify_image_returns_top_3_predictions(
        wildlens_prediction_api_service,
        valid_image_file,
        mocker
, classification_for_10_classes):
    ...
    result = await wildlens_prediction_api_service.classify_image(valid_image_file)
